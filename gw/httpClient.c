...
int buildXML(SMSmsg *msg, char* postdata) {
	...
	strcpy(shortNumber, msg->shortNumber);
	strcpy(carrier, msg->carrier);
	//getFormatedPhone(procConfig.prefix, phone);

	escapeXML(textOut, msg->text);

	sprintf(postdata,"<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>\n<sms-Request>\n<celular>%s</celular>\n<mensaje>%s</mensaje>\n<numero_corto>%s/numero_corto>\n<carrier>%s</carrier>\n</sms-Request>",phone, textOut, shortNumber, carrier);
